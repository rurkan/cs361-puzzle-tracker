...
def change_cancel_time(username):
  ...
  while type(userin) is not int:
    ...
    centerprint_string("Successfully modified cancel delay, new delay: "+str(userin), "-")
    return(["settings",username])
...
def confirmation_screen(confirmation_type, username):
  userin = get_screen_input(settings+""+confirmation_type+".txt", make_dict(usr=username))
  if(confirmation_type == "user_history_reset"):
    if(userin == "confirm_reset"):
      incomplete_screen("DELETED HISTORY")
      return(["ex", None])
    centerprint_string("You did not input one of the available options, auto-cancelling.","-")
    sleep(get_cancel_time(username))
    return(["settings", username])
